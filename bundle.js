...
var _d = __webpack_require__(297);
  ...
  INPUT: function INPUT() {
    ...
    return {
      ...
      apply: function apply() {
        return { out: Math.floor(state++ / (10 * period)) % 2 == 0 };
      }
    };
  },
...
var spec = {
  nodes: ['INPUT', 'INPUT', 'AND', 'XOR', 'OUTPUT', 'OUTPUT'],
  wires: [{ from: '0.out', to: '2.a' }, { from: '1.out', to: '2.b' }, { from: '0.out', to: '3.a' }, { from: '1.out', to: '3.b' }, { from: '2.out', to: '4.in' }, { from: '3.out', to: '5.in' }]
};

var body = d3.select('body').append('svg').classed('main', true).attr('width', 600).attr('height', 600);

var wireSel = body.append('g');
var gateSel = body.append('g');

var gatePosX = [100, 120, 200, 200, 300, 300];
var gatePosY = [100, 200, 100, 200, 100, 200];

var global_gates = spec.nodes.map(function (type, i) {
  return _extends({}, gateSpec[type](), { x: gatePosX[i], y: gatePosY[i] });
});
var global_wires = spec.wires.map(function (wire) {
  return _extends({}, wire, { high: false, last: false });
});

var gateSize = 40;

var renderGates = function renderGates(gates) {
  gateSel.selectAll('rect').data(gates).enter().append('rect').attr('width', gateSize).attr('height', gateSize).attr('x', function (d) {
    return d.x - gateSize / 2;
  }).attr('y', function (d) {
    return d.y - gateSize / 2;
  }).attr('fill', 'blue');
};

var gateIndex = function gateIndex(wireSpec) {
  return parseInt(wireSpec.split('.')[0]);
};
var gatePort = function gatePort(wireSpec) {
  return wireSpec.split('.')[1];
};

var renderWires = function renderWires(gates, wires) {

  var computePath = function computePath(_ref) {
    var from = _ref.from,
        to = _ref.to;

    var a = gates[gateIndex(from)];
    var b = gates[gateIndex(to)];

    var aIndex = a.outputs.indexOf(gatePort(from));
    var bIndex = b.inputs.indexOf(gatePort(to));

    var aOffset = 30 * ((a.outputs.length - 1) / 2 - aIndex);
    var bOffset = 30 * ((b.inputs.length - 1) / 2 - bIndex);
  ...
  wireSel.selectAll('path.background').data(wires).enter().append('path').classed('background', true).attr('d', computePath).attr('stroke', 'grey').attr('fill', 'none');

  wireSel.selectAll('path.foreground').data(wires).enter().append('path').classed('foreground', true).attr('d', computePath).attr('stroke-dasharray', function () {
    var length = this.getTotalLength();
    return '0 ' + length + ' ' + length + ' ' + length;
  }).attr('stroke', 'red').attr('fill', 'none');

  wireSel.selectAll('path.foreground').transition().ease(d3.easeLinear).duration(1000).attr('stroke-dasharray', function (_ref2, i) {
    var high = _ref2.high,
        last = _ref2.last;
    ...
    if (!high && last) {
      ...
      var _length = this.getTotalLength();
      return _length + ' ' + _length + ' ' + _length + ' ' + _length;
    }
    return this.attributes['stroke-dasharray'].value;
  }).on('end', function (_ref3, i) {
    var high = _ref3.high,
        last = _ref3.last;
...
var update = function update(wires, gates) {

  var activePorts = {};
  wires.filter(function (_ref4) {
    var high = _ref4.high;
    return high;
  ...
  gates.forEach(function (gate, i) {
    var params = gate.inputs.map(function (input) {
      return !!(activePorts[i] && activePorts[i][input]);
    });
    var outputs = gate.apply.apply(gate, _toConsumableArray(params));
  ...
  return wires.map(function (_ref6) {
    var from = _ref6.from,
        to = _ref6.to,
        high = _ref6.high;

    var gate = newPorts[gateIndex(from)];
    var newHigh = gate && gate[gatePort(from)];
    var last = high;
    return { from: from, to: to, high: newHigh, last: last };
  });
};

var updateAndRender = function updateAndRender() {
